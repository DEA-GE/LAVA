...
suitability_config_file = os.path.join("configs", "suitability.yaml")
...
if manual_resolutions:
    ...
    pixel_path = os.path.join(
        data_path, f"pixel_size_{region_name}_{local_crs_tag}.json"
    )
...
suitability_params = {
    k for k, v in config_suitability["suitability_params"].items() if v
}
...
if input_area == "available_land":
    for tech in suitability_techs:
        ...
        ref = rasterio.open(
            path
        )  # Should be standardized to a region raster or intersection of all rasters
...
if "terrain" in suitability_params:
    terrain_ruggedness_path = os.path.join(
        data_path,
        "derived_from_DEM",
        f"TerrainRuggednessIndex_{region_name}_{local_crs_tag}.tif",
    )
    terrain_ruggedness = rasterio.open(terrain_ruggedness_path)
    terrain_ruggedness_reproj = align_to_reference(
        terrain_ruggedness, ref, resampling=Resampling.bilinear
    )

    landcover_path = os.path.join(
        data_path,
        f"landcover_{config['landcover_source']}_{region_name}_{local_crs_tag}.tif",
    )
    landcover = rasterio.open(landcover_path)
    land_cover_reproj = align_to_reference(
        landcover, ref, resampling=Resampling.nearest
    )
...
if "substation_distance" in suitability_params:
    substation_distance_path = os.path.join(
        data_from_proximity, "substation_distance.tif"
    )
    substation_distance = rasterio.open(substation_distance_path)
    substation_distance_reproj = align_to_reference(
        substation_distance, ref, resampling=Resampling.bilinear
    )
...
if suitability_params:
    print(
        f"Calculating costmaps based on suitability parameters: {list(suitability_params)}"
    )
    ...
    for tech in suitability_techs:
        ...
        if "terrain" in suitability_params:
            ...
            for terrain_type in config_suitability["terrain_modifier"]:
                ...
                terrain_mask = (terrain_ruggedness_reproj >= lower) & (
                    terrain_ruggedness_reproj < upper
                )
                terrain_factor[terrain_mask] = terrain_type["cost"][tech] - 1

            # Landcover modifiers
            for landcover_type, vals in config_suitability[
                "landcover_modifier"
            ].items():
            ...
            costmap[tech] *= (
                1
                + terrain_factor
                * config_suitability["modifier_weights"]["terrain"][tech]
            )
            ...
            export_raster(
                ...
                os.path.join(
                    output_path,
                    f"terrain_factor_{tech}_{scenario}_{region_name}_{local_crs_tag}.tif",
                ),
                ref,
                local_crs_obj,
            )
        ...
        if "topography" in suitability_params:
            ...
            costmap[tech] *= (
                1
                + topography_factor
                * config_suitability["modifier_weights"]["topography"][tech]
            )
            ...
            export_raster(
                ...
                os.path.join(
                    output_path,
                    f"topography_factor_{tech}_{scenario}_{region_name}_{local_crs_tag}.tif",
                ),
                ref,
                local_crs_obj,
            )
        ...
        if "substation_distance" in suitability_params:
            avg_sub = config_suitability["average_sub_dist"][
                config_suitability["region_set"][region_name]
            ]
            ...
            costmap[tech] *= (
                1
                + substation_factor
                * config_suitability["modifier_weights"]["substation_distance"][tech]
            )
            ...
            export_raster(
                ...
                os.path.join(
                    output_path,
                    f"substation_factor_{tech}_{scenario}_{region_name}_{local_crs_tag}.tif",
                ),
                ref,
                local_crs_obj,
            )
        ...
        if "region" in suitability_params:
            ...
            costmap[tech] *= (
                1
                + region_factor * config_suitability["modifier_weights"]["region"][tech]
            )

        # --- Export cost maps ---
        export_raster(
            costmap[tech] * potential[tech],
            os.path.join(
                output_path,
                f"costmap_{tech}_{scenario}_available_{region_name}_{local_crs_tag}.tif",
            ),
            ref,
            local_crs_obj,
        )
...
if multi_tech:
    RG_comb = list(
        itertools.product(*RG.values())
    )  # All combinations of resource grades across technologies
...
if suitability_params:
    for tech in suitability_techs:
        df_tier_potentials[tech] = pd.DataFrame(
            index=areas, columns=config_suitability["tiers"].keys()
        )
else:
    for tech in suitability_techs:
        df_tier_potentials[tech] = pd.DataFrame(
            index=areas, columns=["Potential share"]
        )

# Total available land area
total_avail = (
    np.sum(union([potential[tech] for tech in suitability_techs])) * pixel_area_km2
)
...
for tech in suitability_techs:
    ...
    if tech in ["onshorewind", "offshorewind"]:
        tech_grades[tech] = {
            rg: filter(
                potential_filtered[tech],
                GWA_reproj,
                tech_configs[tech]["rg_thr"][rg][0],
                tech_configs[tech]["rg_thr"][rg][1],
            )
            for rg in RG[tech]
        }
    elif tech == "solar":
...
for tech in suitability_techs:
    for rg in RG[tech]:
        ...
        if inclusion_area.sum() < min_size_rg:
            print(
                f"Potential found for {rg} in {region_name} is below minimum. Adding to distributed area."
            )
            tech_grades[tech]["distributed"] = union(
                [tech_grades[tech]["distributed"], inclusion_area]
            )
            continue

        # Save potential area and export raster
        df_potentials.loc[f"{region_name}_{rg}", "Potential"] = (
            np.sum(inclusion_area) * pixel_area_km2
        )
        export_raster(
            inclusion_area,
            os.path.join(
                output_path, f"{region_name}_{rg}_{scenario}_{local_crs_tag}.tif"
            ),
            ref,
            local_crs_obj,
        )
        ...
        if suitability_params:
            for t in config_suitability["tiers"]:
                tier_area = filter(
                    inclusion_area,
                    costmap[tech],
                    config_suitability["tiers"][t][0],
                    config_suitability["tiers"][t][1],
                )
                df_tier_potentials[tech].loc[f"{region_name}_{rg}", t] = np.sum(
                    tier_area
                ) / np.sum(inclusion_area)
        else:
...
if multi_tech:
    for tech_combo in RG_comb:
        label = ", ".join(
            f"{tech}: {rg}" for tech, rg in zip(suitability_techs, tech_combo)
        )
        print(f"Processing combination: {label}")

        # Find inclusion area by overlapping tech grades
        arrays = [
            tech_grades[tech][rg] for tech, rg in zip(suitability_techs, tech_combo)
        ]
        inclusion_area = overlap(arrays)

        if inclusion_area.sum() < min_size_rg:
            print(
                f"Potential found for combination {tech_combo} and in {region_name} is below minimum. Adding to distributed area."
            )
            for tech in suitability_techs:
                tech_grades[tech]["distributed"] = union(
                    [tech_grades[tech]["distributed"], inclusion_area]
        ...
        # Save potential area and export raster
        df_potentials.loc[f"{region_name}_" + "_".join(tech_combo), "Potential"] = (
            np.sum(inclusion_area) * pixel_area_km2
        )
        export_raster(
            inclusion_area,
            os.path.join(
                ...
                f"{region_name}_"
                + "_".join(tech_combo)
                + f"_{scenario}_{local_crs_tag}.tif",
            ),
            ref,
            local_crs_obj,
        )
        ...
        if suitability_params:
            for t in config_suitability["tiers"]:
                for tech in suitability_techs:
                    ...
                    df_tier_potentials[tech].loc[
                        f"{region_name}_" + "_".join(tech_combo), t
                    ] = np.sum(tier_area) / np.sum(inclusion_area)
        else:
            for tech in suitability_techs:
                df_tier_potentials[tech].loc[
                    f"{region_name}_" + "_".join(tech_combo), "Potential share"
                ] = 1


# Process the distributed areas found above
distributed_area = union(
    [tech_grades[tech]["distributed"] for tech in suitability_techs]
)
df_potentials.loc[f"{region_name}_distributed", "Potential"] = (
    np.sum(distributed_area) * pixel_area_km2
)
export_raster(
    distributed_area,
    os.path.join(
        output_path, f"{region_name}_distributed_{scenario}_{local_crs_tag}.tif"
    ),
    ref,
    local_crs_obj,
)
...
if suitability_params:
    for t in config_suitability["tiers"]:
        for tech in suitability_techs:
            ...
            df_tier_potentials[tech].loc[f"{region_name}_distributed", t] = np.sum(
                tier_area
            ) / np.sum(distributed_area)
else:
    for tech in suitability_techs:
        df_tier_potentials[tech].loc[
            f"{region_name}_distributed", "Potential share"
        ] = 1
...
potentials_file = os.path.join(
    output_path, f"{region_name}_{scenario}_resource_grade_potentials.csv"
)
df_potentials.to_csv(potentials_file)
for tech in suitability_techs:
    df_tier_potentials_file = os.path.join(
        output_path, f"{region_name}_{tech}_{scenario}_tier_potentials.csv"
    )
...
relevant_resource_grades_file = os.path.join(
    output_path, f"{region_name}_{scenario}_relevant_resource_grades.json"
)
with open(relevant_resource_grades_file, "w") as f:
    json.dump(relevant_resource_grades, f)
for tech in suitability_techs:
    relevant_resource_grades_tech = (
        df_tier_potentials[tech].dropna(how="all").index.tolist()
    )
    relevant_resource_grades_tech_file = os.path.join(
        output_path, f"{region_name}_{tech}_{scenario}_relevant_resource_grades.json"
    )
