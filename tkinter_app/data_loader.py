...
CONFIG_SECTION_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "parameters": [
            ...
            {"key": "GADM_level", "type": "integer", "description": "GADM admin level."},
    ...
    },
    {
        ...
        "parameters": [
            {"key": "population_source", "type": "string", "description": "Population source (worldpop, file, or 0)."},
            {"key": "population_year", "type": "integer", "description": "WorldPop reference year."},
        ],
    },
    {
        "name": "resources",
    ...
    },
    {
        ...
        "parameters": [
            {"key": "forest_density_source", "type": "source", "description": "Use 0 to disable or provide a forest-density source."},
    ...
    },
    {
        ...
        "parameters": [
            {"key": "weather_external_data_path", "type": "string", "description": "Optional external weather data directory."},
            {"key": "input_area", "type": "string", "description": "Area used to create energy profiles."},
            {"key": "weather_year", "type": "number", "description": "Weather dataset year."},
            {"key": "weather_data_extend", "type": "string", "description": "Weather download extent mode."},
            {"key": "weather_data_geo_bounds", "type": "mapping", "description": "Geographical bounds used when geo_bounds is selected."},
            {"key": "weather_bias_correction", "type": "mapping", "description": "Bias-correction toggles by technology."},
            {"key": "weather_bias_range", "type": "array", "description": "Minimum and maximum bias-correction factors."},
        ],
    },
    {
...
ONSHORE_SECTION_DEFINITIONS: List[Dict[str, Any]] = [
    {
        ...
        "parameters": [
            ...
            {"key": "projection_manual", "type": "string", "description": "Optional manual projection override."},
        ],
    },
    {
        ...
        "parameters": [
            {"key": "landcover_codes", "type": "mapping", "description": "Landcover buffers by code (m)."},
        ],
    },
    {
        "name": "dem_thresholds",
        ...
        "parameters": [
            ...
            {"key": "north_facing_pixels", "type": "number", "description": "Enable north-facing pixel exclusion."},
            {"key": "max_population", "type": "number", "description": "Maximum population per pixel."},
    ...
    },
    {
        ...
        "parameters": [
            {"key": "rg_thr", "type": "mapping", "description": "Wind resource group thresholds (m/s)."},
        ],
    },
]
...
SOLAR_SECTION_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "parameters": [
            {"key": "rg_thr", "type": "mapping", "description": "Solar resource group thresholds (kWh/m2/yr)."},
        ],
    },
]
...
CONFIG_SNAKEMAKE_SECTION_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "parameters": [
            {"key": "study_region_name", "type": "array", "description": "Region names for the run."},
...
def load_snakemake_sections() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        return _build_sections_from_data(flattened, CONFIG_SNAKEMAKE_SECTION_DEFINITIONS)
    except Exception:
        return deepcopy(fallback)
...
def validate_configuration_documents(
    ...
    if "config.yaml" in documents:
        ...
        if weather_extend and weather_extend not in {"study_region", "geo_bounds", "country_code"}:
            add(
                "error",
                "config.yaml",
                "weather_data_extend",
        ...
        if weather_extend == "geo_bounds":
            ...
            if not isinstance(bounds, Mapping) or any(not nonempty(bounds.get(k)) for k in required_bounds):
                add(
                    "error",
                    "config.yaml",
                ...
                try:
                    west, south, east, north = (float(bounds[k]) for k in required_bounds)
                    valid_order = west < east and south < north
                    valid_range = -180 <= west <= 180 and -180 <= east <= 180 and -90 <= south <= 90 and -90 <= north <= 90
                except (TypeError, ValueError):
    ...
    if "snakemake.yaml" in documents:
        ...
        if not isinstance(technologies, Sequence) or isinstance(technologies, (bytes, bytearray)):
            technologies = []
        technologies = [str(value).strip() for value in technologies if str(value).strip()]
        if not technologies:
            add("error", "snakemake.yaml", "technologies", "Select at least one technology.")
        for technology in technologies:
            file_name = f"{technology}.yaml"
            if file_name not in documents and not (Path(configs_path) / file_name).exists():
                add(
                    "error",
                    "snakemake.yaml",
                    "technologies",
        ...
        dependencies = {
            ...
            "energy_profiles": ("suitability", "weather_data_prep", "weather_bias_adjust"),
        }
