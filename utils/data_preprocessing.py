...
def suitability_input_check(config_path="configs/config.yaml", suitability_config_path="configs/suitability.yaml", project_root="."):
    """
    Check if all necessary available_land .tif files exist for suitability analysis.
    
    Raises an error if any required files are missing, instructing the user on what to do.
    
    Parameters
    ----------
    config_path : str
        Path to config.yaml
    suitability_config_path : str
        Path to suitability.yaml
    project_root : str
    ...
    ------
    FileNotFoundError
        If any required available_land files are missing
    """
    # Load config files
    with open(os.path.join(project_root, config_path), 'r', encoding='utf-8') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    
    with open(os.path.join(project_root, suitability_config_path), 'r', encoding='utf-8') as f:
        config_suitability = yaml.load(f, Loader=yaml.FullLoader)
    
    # Get regions and ensure it's a list
    regions = config.get('study_region_name', [])
    if isinstance(regions, str):
        regions = [regions]
    
    # Get suitability technologies
    suitability_techs = config_suitability.get('suitability_techs', [])
    scenario = config.get('scenario', 'ref')
    
    # Determine CRS tag (use EPSG4326 as fallback)
    crs_tag = "EPSG4326"
    
    missing_files = []
    
    # Check for each region and technology combination
    for region in regions:
        region_clean = clean_region_name(region)
        
        for tech in suitability_techs:
            # Construct expected filename
            filename = f"{region_clean}_{tech}_{scenario}_available_land_{crs_tag}.tif"
            filepath = os.path.join(project_root, 'data', region_clean, 'available_land', filename)
            
            if not os.path.exists(filepath):
                missing_files.append({'region': region_clean, 'technology': tech, 'file': filename, 'expected_path': filepath})
    
    # Raise error if files are missing
    if missing_files:
        error_msg = (
            f"\n{'='*80}\n"
            f"ERROR: Missing available_land files required for suitability analysis\n"
            f"{'='*80}\n\n"
            f"Configuration:\n"
            f"  - Regions: {', '.join(regions)}\n"
            f"  - Suitability technologies: {', '.join(suitability_techs)}\n"
            f"  - Scenario: {scenario}\n\n"
        ...
        for item in missing_files:
            ...
            error_msg += f"    Expected location: data/{item['region']}/available_land/\n\n"
        
        error_msg += (
            f"HOW TO FIX:\n"
            f"  These files are generated by the exclusion stage of the pipeline.\n"
            f"  Make sure you have run the following stages in order:\n"
            f"    1. spatial_data_prep\n"
            f"    2. exclusion\n\n"
            f"  Then run suitability again.\n\n"
        )
        error_msg += f"{'='*80}\n"
