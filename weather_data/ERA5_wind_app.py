...
class WindMapApp:
    ...
    def setup_gui(self):
        ...
        self.file_label = ttk.Label(
            top_frame,
            text="Drag and drop a NetCDF file here",
            foreground="gray",
        ...
        # View mode dropdown
        ttk.Label(control_frame, text="View Mode:").grid(
        ...
        self.minus_btn = ttk.Button(
            self.timestep_frame, text="-", width=3, command=self.decrease_timestep
        )
        self.minus_btn.pack(side=tk.LEFT, padx=2)

        self.timestep_entry = ttk.Entry(
            self.timestep_frame,
            textvariable=self.current_timestep,
            width=8,
            justify=tk.CENTER,
        )
        ...
        self.plus_btn = ttk.Button(
            self.timestep_frame, text="+", width=3, command=self.increase_timestep
        )
        ...
        self.animate_btn = ttk.Button(
            self.timestep_frame, text="Start Animation", command=self.toggle_animation
        )
        ...
        self.coord_label = ttk.Label(
            self.coord_frame, text="Selected: --", font=("Courier", 9)
        )
    ...
    def setup_map_plot(self):
        ...
        self.map_toolbar = NavigationToolbar2Tk(
            self.map_canvas, self.map_frame, pack_toolbar=False
        )
    ...
    def setup_ts_plot(self):
        ...
        ttk.Label(self.ts_range_frame, text="Show timesteps from").pack(
            side=tk.LEFT, padx=(0, 2)
        )
        self.ts_first_entry = ttk.Entry(
            self.ts_range_frame,
            textvariable=self.ts_first_timestep,
            width=6,
            justify=tk.CENTER,
        )
        self.ts_first_entry.pack(side=tk.LEFT)
        ttk.Label(self.ts_range_frame, text="to").pack(side=tk.LEFT, padx=(2, 2))
        self.ts_last_entry = ttk.Entry(
            self.ts_range_frame,
            textvariable=self.ts_last_timestep,
            width=6,
            justify=tk.CENTER,
        )
        ...
        self.ts_ax.text(
            0.5,
            0.5,
            "Click on the map to see time series",
            ha="center",
            va="center",
            fontsize=12,
            color="gray",
    ...
    def plot_map(self):
        ...
        if self.view_mode.get() == "Mean":
            ...
            time_value = data_to_plot["time"].values  # noqa: F841
        ...
        # Add colorbar
        self.colorbar = self.map_fig.colorbar(
            mesh, ax=self.map_ax, orientation="horizontal", pad=0.05, shrink=0.8
        )
        ...
        self.map_ax.set_title(
            f"{title_suffix} ({self.wind_var})", fontsize=12, fontweight="bold"
        )
    ...
    def on_map_click(self, event):
        ...
        try:
            self.selected_point = self.wind_data.sel(
                {self.x_name: x_click, self.y_name: y_click}, method="nearest"
            )
            ...
            self.coord_label.config(text=f"Selected: ({actual_x:.3f}, {actual_y:.3f})")
    ...
    def on_map_hover(self, event):
            ...
            if self.wind_mean is not None:
                ...
                if self.selected_point is not None:
                    ...
                    self.coord_label.config(
                        text=f"Selected: ({sel_x:.3f}, {sel_y:.3f})"
                    )
                else:
                    self.coord_label.config(text="Selected: --")
            self.map_canvas.draw_idle()
        ...
        try:
            ...
            hover_point = data_for_hover.sel(
                {self.x_name: x_hover, self.y_name: y_hover}, method="nearest"
            )
            ...
            self.hover_patch = Rectangle(
                (actual_hover_x - dx / 2, actual_hover_y - dy / 2),
                dx,
                dy,
                linewidth=1,
                edgecolor="blue",
    ...
    def plot_time_series_selected(self, x=None, y=None):
        ...
        if len(values) > 0:
            self.ts_ax.text(
                0.02,
                0.98,
                f"Mean wind speed = {mean_val:.2f} m/s",
        ...
        self.ts_ax.set_title(
            f"Time Series at ({x:.2f}°, {y:.2f}°)", fontsize=11, fontweight="bold"
        )
